            ...
            if (LocalServerManager.localServer != null)
            {
                LocalServerManager.localServer.StatusChanged += async (o, e) =>
                {
                    ...
                    gpService.StatusChanged += async (svc, args) =>
                    {
                        ...
                        {
                            // 在本地服务器中获取该服务的URL
                            var gpSvcUrl = (svc as LocalGeoprocessingService).Url.AbsoluteUri + "\\FlowDirection.gpk";
                            // 创建一个Task
                            ...
                            string pathToRaster = @"c:\users\administrator\documents\arcgis\localServer\result.tif";
                        ...
                        try
                        {
                            ...
                            GeoprocessingRaster resultRaster = geoprocessingResult.Outputs["flowDirRaster"] as GeoprocessingRaster;
